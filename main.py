...
app = Flask(__name__)
app.config['SECRET_KEY'] = b'ASD124578'

cars = [
    {'id':1 , 'brand':'Totota' , 'model':'Yaris Ativ' , 'year': 2024 , 'price':50000},
    {'id':2 , 'brand':'Yamaha' , 'model':'Yaris Cross' , 'year': 2025 , 'price':60000},
    {'id':3 , 'brand':'Nissan' , 'model':'Kicls' , 'year': 2026 , 'price':70000}
    ]
    ...
    if request.method == 'POST':
        ...
        for car in cars:
            if brand in car['brand']:
                tmp_cars.append(car)
        return render_template('cars/cars.html', title='Show Cars by Brand', cars=tmp_cars)
...
def new_cars():
    if request.method == 'POST':
        ...
        cars.append(car)
        flash('Add new car successfully' , '')
    ...
    if request.method == 'POST':
        ...
        for car in cars :
            if id == car['id']:
                ...
                car['price'] = price
                break
        flash('Update new car successfully' , 'success')
        return redirect(url_for('show_cars'))
